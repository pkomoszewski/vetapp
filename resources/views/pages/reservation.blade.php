          ...
          <div class="card-body">
            <h5 class="card-title text-muted text-uppercase text-center">{{$data->tytul}}</h5>
                <h6 class="card-price text-center">
                    {{DateTime::createFromFormat('Y-m-d',$data->data_koncertu)->format('d-m-Y')}}</h6>
                ...
                <p> {{$data->opis}}</p>
                @auth
                <a href="order/{{$data->id}}/{{Auth::user()->id}}"
                    class="btn btn-block btn-primary text-uppercase">Zamów</a>
